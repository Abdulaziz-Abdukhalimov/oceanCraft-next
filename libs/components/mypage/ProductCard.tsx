		...
		return (
			<Stack className="product-card-box">
				...
				<Stack className="information-box" onClick={() => pushProductDetail(product?._id)}>
					...
					<Typography className="category">{product.productCategory.replace(/_/g, ' ')}</Typography>
					<Typography className="price">
						<strong>
							{product.productCurrency}
							{formatterStr(product?.productPrice)}
						</strong>
						{product.productPriceType === 'RENT' && product.productRentPeriod && (
							<span className="rent-period"> / {product.productRentPeriod.toLowerCase()}</span>
						)}
					</Typography>
				</Stack>
